...
def parse_nl(text, ing_keys):
    ...
    for m in MEAL_MAP:
        if m in text_l:
            meal = m
            break
    # ingredient extraction — match MULTI-WORD phrases from the index, not tokens
    found = []
    # Check if user phrase appears INSIDE a key (e.g. "ground beef" in "1 lb ground beef")
    # or key appears in user text. Sort keys by length desc so longer phrases win.
    for k in sorted(ing_keys, key=len, reverse=True):
        kl = k.lower()
        if kl in text_l or text_l in kl:
            if kl not in [f.lower() for f in found]:
                found.append(k)
    # also catch single-word ingredients: check if token is a word inside any key
    tokens = re.findall(r"[a-z]+", text_l)
    for t in tokens:
        if len(t) < 3:
            continue
        if t not in " ".join(found).lower():
            # token is a substring of a key word (e.g. "beef" in "1 lb ground beef")
            for k in sorted(ing_keys, key=len, reverse=True):
                kl = k.lower()
                if t in kl.split() or t == kl or t in kl:
                    if kl not in [f.lower() for f in found]:
                        found.append(k)
                        break
        ...
        if ing_set:
            for ing in ing_set:
                ...
                if rec_name in recipes_with:
                    ...
                    missing.append(ing)
            # HARD REQUIRE: every specified ingredient must be present (no silent drop)
            if missing:
                missing = list(ing_set - set(matched))
            total = len(ing_set)
            have = len(matched)
            match_pct = round(have / total * 100)
            # if ANY specified ingredient is entirely missing, this is not a match for "I have X"
