import { assertGatewayExposureConfig, assertHermesApiConfig, assertRealtimeProviderConfig, publicBaseUrl, } from "./config.js";
import { hermesApprovalCompatibility, unnegotiatedHermesApprovalCompatibility, } from "./application/live-gateway/hermes-approval-compatibility.js";
import { HermesClient, REQUIRED_HERMES_SESSION_FEATURES, } from "./adapters/outbound/hermes/hermes-runs.client.js";
import { errorToMessage } from "./domain/error-message.js";
export async function buildReadinessReport(config, options = {}) {
...
async function checkTaskRuntime(tasks) {
    if (!tasks)
        return { ok: true, checked: false, durable: true };
    try {
        await tasks.health();
...
function checkGatewayConfig(config) {
    const base = {
...
async function checkHermesConfig(config, options) {
    ...
    try {
        ...
        const missingSessionFeatures = REQUIRED_HERMES_SESSION_FEATURES.filter((name) => features[name] !== true);
        if (missingSessionFeatures.length > 0) {
            throw new Error(`Hermes API Server is missing required session features: ${missingSessionFeatures.join(", ")}`);
        }
        return {
            ok: true,
            ...base,
...
function checkRealtimeConfig(config, options) {
...
function realtimeCheckSummary(config) {
    const base = {
