...
export async function runLiveProviderSmoke(config, options = {}) {
    ...
    let closeEvent;
    let closing = false;
    let resolveOpen = () => undefined;
    const openSignal = new Promise((resolve) => {
        resolveOpen = resolve;
    });
    ...
    let resolveToolCall;
    let resolveReceipt;
    const toolCallSignal = new Promise((resolve) => {
        resolveToolCall = resolve;
    });
    const receiptSignal = new Promise((resolve) => {
        resolveReceipt = resolve;
    });
    const failFunctionalCheck = (message) => {
        if (functionalSettled)
            return;
        functionalSettled = true;
        resolveToolCall({ error: message });
        resolveReceipt({ error: message });
    };
    const connectTimeoutMessage = `${config.realtime.provider} realtime session did not connect within ${timeoutMs}ms.`;
    ...
    try {
        pendingConnect = adapter.connect({
            sessionId,
            systemInstruction: options.verifyToolCall
            ...
            callbacks: {
                ...
                onClose: (event) => {
                    ...
                    if (!closing)
                        failFunctionalCheck("Local voice closed before the functional check completed.");
                },
                onError: (_error) => {
                    if (!closing) {
                        providerError = true;
                ...
                onEvent: (event) => {
                    ...
                    if (events.length < MAX_RETAINED_SMOKE_EVENTS)
                        events.push(summarizeLiveEvent(event));
                    if (!options.verifyToolCall || functionalSettled)
                        return;
        ...
        session = await withTimeout(pendingConnect, timeoutMs, connectTimeoutMessage);
        if (!openCallback) {
        ...
        if (options.verifyToolCall) {
            ...
            await session.sendText("Delegate this exact task in the background: reply with exactly PROVIDER SMOKE OK.");
            const toolResult = await withFunctionalTimeout(toolCallSignal, timeoutMs, "Local voice did not emit the required task tool call before the functional-check deadline.");
            ...
            if (toolResult.call.name !== "start_background_task"
                || typeof toolResult.call.args.message !== "string"
                || !toolResult.call.args.message.toUpperCase().includes("PROVIDER SMOKE OK")) {
                throw new ProviderFunctionalSmokeError("Local voice did not preserve the requested work in its task tool call during setup.");
            }
            ...
            const receiptResult = await withFunctionalTimeout(receiptSignal, timeoutMs, "Local voice did not produce the spoken task receipt before the functional-check deadline.");
        ...
        if (error instanceof ProviderFunctionalSmokeError)
            throw error;
...
async function withFunctionalTimeout(promise, timeoutMs, message) {
    try {
...
function summarizeCloseEvent(event) {
...
//# sourceMappingURL=live-provider-smoke.js.map