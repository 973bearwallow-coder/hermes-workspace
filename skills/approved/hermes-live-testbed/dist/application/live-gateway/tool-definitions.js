...
const HERMES_LIVE_TOOL_DEFINITIONS = [
    {
        ...
        description: "Send one conversational turn to the Hermes session selected by the user. Use it for answers, memory, and follow-ups that must remain in that persisted chat; use a background task for long independent work.",
    ...
    },
    {
        ...
        description: "Delegate meaningful work to Hermes Agent as a durable background task. Returns quickly; the user may keep talking or disconnect while the task continues.",
        parametersJsonSchema: {
            type: "object",
            additionalProperties: false,
            properties: {
                ...
                execution_mode: {
                    ...
                    description: "Use exclusive unless the task is provably read-only. Read-only tasks overlap only when their resource_keys are disjoint; mutating tasks are serialized.",
                },
                resource_keys: {
                    type: "array",
                    maxItems: 8,
                    items: { type: "string" },
                    description: "Stable resources read or touched by the task, such as an absolute repository path or deployment target. Tasks sharing a key never overlap.",
                },
            },
            required: ["message"],
        },
    ...
    },
    {
        ...
        description: "Start durable follow-up work from a finished task and its retained result. Use the exact task_id returned by the gateway. The follow-up is a new independently stoppable task in the same lineage.",
        parametersJsonSchema: {
            type: "object",
            additionalProperties: false,
            properties: {
    ...
    },
    {
        ...
        description: "Pause microphone listening only when the user explicitly asks to pause, mute, or stop listening. This keeps Live Voice connected and leaves every background task running; the user resumes from the client control.",
        parametersJsonSchema: {
            type: "object",
            additionalProperties: false,
            properties: {},
...
export function selectHermesLiveToolDeclarations(names) {
    if (names === undefined)
        return HERMES_LIVE_TOOL_DECLARATIONS;
    const allowed = new Set(names);
    return HERMES_LIVE_TOOL_DECLARATIONS.filter((tool) => allowed.has(tool.name));
}
export function selectOpenAIHermesLiveTools(names) {
    if (names === undefined)
        return OPENAI_HERMES_LIVE_TOOLS;
    const allowed = new Set(names);
...
export function selectCompactOpenAIHermesLiveTools(names) {
...
function withoutJsonSchemaDescriptions(value) {
    if (Array.isArray(value))
        return value.map(withoutJsonSchemaDescriptions);
