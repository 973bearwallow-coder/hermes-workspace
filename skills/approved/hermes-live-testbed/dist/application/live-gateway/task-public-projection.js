...
export function projectTaskSnapshot(record, options = {}) {
    ...
    if (state === "completed") {
        ...
        if (summary)
            snapshot.progress = { message: summary };
    }
    return snapshot;
}
export function projectTaskLifecycle(record, requestId) {
    const base = {
        taskId: record.taskId,
        sequence: record.sequence,
...
export function projectTaskNotification(record) {
 ...
 */
export function projectSupersededTaskNotification(record) {
    if (notificationKind(record.status) || record.notification.unread)
        return undefined;
...
export function notificationIdForTask(record) {
    return notificationId(record.taskId, notificationAnchor(record).sequence);
}
export function isTaskNotificationState(status) {
    return notificationKind(status) !== undefined;
}
function publicTaskState(record) {
    if (record.stopRequestedAt !== undefined
...
function publicTaskError(record) {
...
function notificationKindFromEvent(type) {
    switch (type) {
        case "completed":
        case "failed":
        case "cancelled":
...
function notificationId(taskId, sequence) {
    return `notification_${taskId}_${sequence}`;
}
function notificationAnchor(record) {
...
function notificationMessage(record, kind) {
...
//# sourceMappingURL=task-public-projection.js.map