...
export class TerminalGatewaySession {
    ...
    tasks = new Map();
    taskLifecycleSequences = new Map();
    taskLifecycleRevisions = new Map();
        ...
        await new Promise((resolve, reject) => {
            ...
            socket.on("message", (raw) => {
                try {
                    ...
                    if (!this.ready)
                        settle(new Error(sanitizeTerminalText(message, 500)));
            ...
            socket.once("error", () => {
                if (!this.ready)
                    settle(new Error("Could not connect to the Hermes Live gateway."));
                else
                    this.line("[connection] Gateway WebSocket error; the session may close.");
            });
            socket.once("close", (code) => {
                ...
                if (!wasReady)
                    settle(new Error(`Gateway closed before session.ready (code ${code}).`));
                if (wasReady && !this.intentionalClose) {
                    this.line(`[connection] Disconnected (code ${code}). Background tasks are server-owned and may still be running; reconnect to check them.`);
                }
    ...
    execute(input) {
        ...
        if (!line.startsWith("/")) {
            ...
            if (this.send({ type: "text.input", id: this.nextRequestId(), text: line }))
                this.line("[you] Sent.");
            return { closeRequested: false };
        }
        ...
        switch (command) {
            ...
            case "/status":
                if (args[0])
                    this.requestTask(args[0], "status");
                else
                    this.printStatus();
                break;
            case "/result":
                if (args[0])
                    this.requestTask(args[0], "result");
                else
                    this.line("[input] Usage: /result <taskId>.");
                break;
            case "/followup": {
                const [taskId, ...messageParts] = args;
                if (taskId && messageParts.length > 0)
                    this.requestFollowUp(taskId, messageParts.join(" "));
                else
                    this.line("[input] Usage: /followup <taskId> <message>.");
                break;
            }
            case "/ack":
            case "/read":
                if (args[0])
                    this.requestNotificationAcknowledgement(args[0]);
                else
                    this.line(`[input] Usage: ${command} <taskId>.`);
                break;
            case "/interrupt":
                this.responseActive = false;
                this.send({
            ...
            case "/stop":
                if (args[0])
                    this.requestStop(args[0]);
                else
                    this.line("[input] Usage: /stop <taskId>. Name the exact task; /interrupt only stops provider speech.");
                break;
            case "/quit":
            case "/exit":
    ...
    handleSessionReady(message) {
        if (!message.conversation)
            throw new Error("Protocol v4 session.ready is missing Hermes conversation state.");
        this.ready = true;
    ...
    handleServerMessage(message) {
        switch (message.type) {
            ...
            case "task.cancelled":
            case "task.unknown":
                if (this.applyTaskLifecycle(message))
                    this.renderTaskLifecycle(message);
                return;
            case "session.error": {
    ...
    handleTaskSnapshot(message) {
        ...
        if (pending?.kind === "result") {
            ...
            if (!task)
                this.line(`[Hermes ${pending.taskId}] Task not found.`);
            else
        ...
        if (message.reason === "list")
            this.renderTaskList(message.tasks, message.truncated);
    }
    applyTaskLifecycle(message) {
    ...
    taskFromLifecycle(existing, message) {
        ...
        switch (message.type) {
            case "task.accepted":
                ...
                if (message.title)
                    base.title = message.title;
                if (message.kind)
                    base.kind = message.kind;
                if (message.parentTaskId)
                    base.parentTaskId = message.parentTaskId;
                if (message.rootTaskId)
                    base.rootTaskId = message.rootTaskId;
                break;
            case "task.started":
                base.state = "running";
    ...
    requestFollowUp(taskIdInput, messageInput) {
        const taskId = validTaskId(taskIdInput);
    ...
    requestNotificationAcknowledgement(taskIdInput) {
        const taskId = validTaskId(taskIdInput);
        if (!taskId) {
            this.line(`[input] Invalid task id ${singleLine(taskIdInput, 120)}.`);
            return;
    ...
    trackAndSend(id, pending, message) {
    ...
    upsertTaskSnapshot(task) {
        ...
        if (task.sequence < lifecycleSequence)
            return;
    ...
    handleTaskNotification(message) {
        ...
        if (message.requestId) {
            ...
            if (pending.taskId !== message.taskId ||
    ...
    renderTaskList(tasks, truncated) {
        if (tasks.length === 0) {
    ...
    flushAssistantResponse() {
        ...
        if (transcript)
            this.line(prefixMultiline("[voice] ", transcript));
...
export function normalizeGatewayWebSocketUrl(value) {
    ...
    if (url.hash)
        throw new Error("HERMES_LIVE_URL must not contain a URL fragment.");
...
export function sanitizeTerminalText(value, maxChars = MAX_RENDERED_TEXT_CHARS) {
...
function parseServerMessage(raw) {
    ...
    if (record.type === "session.ready" && record.protocolVersion !== HERMES_LIVE_PROTOCOL_VERSION) {
        throw new Error(`Gateway protocol mismatch: expected v${HERMES_LIVE_PROTOCOL_VERSION}, received ${String(record.protocolVersion)}.`);
    }
...
function prefixMultiline(prefix, value) {
...
function lifecycleRevision(message) {
...
function assertCompatibleTaskRevision(retained, incoming) {
    const conflict = (field) => {
        throw new Error(`Gateway sent conflicting ${field} for ${incoming.taskId} at sequence ${incoming.sequence}.`);
    };
    if (retained.taskId !== incoming.taskId)
        conflict("task id");
    if (retained.state !== incoming.state)
        conflict("task state");
    ...
    if (retained.error && incoming.error) {
        if (!isDeepStrictEqual(retained.error, incoming.error))
            conflict("task error");
    }
    else if (Boolean(retained.error) !== Boolean(incoming.error) &&
...
function mergeEqualSequenceTask(retained, incoming) {
    ...
    if (next.kind === undefined && incoming.kind !== undefined)
        next.kind = incoming.kind;
    if (next.parentTaskId === undefined && incoming.parentTaskId !== undefined)
        next.parentTaskId = incoming.parentTaskId;
    if (next.rootTaskId === undefined && incoming.rootTaskId !== undefined)
        next.rootTaskId = incoming.rootTaskId;
    if (next.startedAt === undefined && incoming.startedAt !== undefined)
        next.startedAt = incoming.startedAt;
    if (next.finishedAt === undefined && incoming.finishedAt !== undefined)
        next.finishedAt = incoming.finishedAt;
    if (next.progress === undefined && incoming.progress !== undefined)
        next.progress = { ...incoming.progress };
    if (incoming.result) {
        if (!next.result) {
