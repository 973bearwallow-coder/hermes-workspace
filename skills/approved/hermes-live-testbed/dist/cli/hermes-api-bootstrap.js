...
export function resolveHermesHome(home, env) {
    return resolve(env.HERMES_HOME || join(home, ".hermes"));
}
export function generateHermesApiKey() {
    return randomBytes(32).toString("hex");
}
export function isDefaultLocalHermesApi(value) {
    try {
 ...
 */
export async function ensureHermesApiEnvironment(hermesHome, apiKey) {
    ...
    const handle = await open(temporaryPath, fsConstants.O_CREAT | fsConstants.O_EXCL | fsConstants.O_WRONLY, ENV_FILE_MODE);
    try {
...
async function readSafeEnvironment(path) {
    ...
    if (stat.size > MAX_ENV_BYTES)
        throw new Error(`Hermes environment exceeds ${MAX_ENV_BYTES} bytes.`);
...
//# sourceMappingURL=hermes-api-bootstrap.js.map