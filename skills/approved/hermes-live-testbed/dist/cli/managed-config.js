...
const managedKeySet = new Set(MANAGED_CONFIG_KEYS);
export function managedConfigPath(options = {}) {
    return resolve(options.path
        || process.env.HERMES_LIVE_CONFIG_FILE
        || join(options.hermesHome
            ?? process.env.HERMES_HOME
            ?? join(options.home ?? homedir(), ".hermes"), "hermes-live", "config.env"));
}
export async function applyManagedConfigToProcess(options = {}) {
...
export async function resolvedManagedEnvironment(env = process.env, options = {}) {
    const result = await readManagedConfig(options);
    return { ...result.values, ...env };
}
export async function readManagedConfig(options = {}) {
...
export async function writeManagedConfig(values, options = {}) {
    const path = managedConfigPath(options);
...
export function parseManagedConfig(source, sourceLabel = "managed config") {
...
export function serializeManagedConfig(values) {
    const lines = [
...
async function prepareConfigDirectory(directory) {
...
async function assertSafeConfigDirectory(directory) {
    const stat = await lstat(directory);
    if (stat.isSymbolicLink() || !stat.isDirectory()) {
        throw new Error(`Managed config directory must be a real directory, not a symlink: ${directory}`);
    }
...
function assertSafeFileStat(path, stat) {
...
function assertOwnedByCurrentUser(path, uid) {
