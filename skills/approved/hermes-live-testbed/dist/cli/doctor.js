...
import { MIN_MANAGED_LOCAL_MEMORY_BYTES, probeLocalVoiceEndpoint, } from "./local-voice.js";
...
import { classifyHermesVersion, HERMES_COMPATIBILITY, parseHermesVersion, } from "../hermes-compatibility.js";
const packageRequire = createRequire(import.meta.url);
const PACKAGE_VERSION = packageRequire("../../package.json").version;
...
export function parseDoctorOptions(args) {
    const options = { json: false, providerSmoke: false };
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (argument === "--json")
            options.json = true;
        else if (argument === "--provider-smoke")
            options.providerSmoke = true;
...
export async function runDoctor(options, dependencies = {}) {
    ...
    if (!plugin.installed || !plugin.manifestFound) {
        ...
        checks.push(installedVersion === PACKAGE_VERSION
            ...
            : fail("plugin", "Hermes plugin", `Installed ${installedVersion ? `v${installedVersion}` : "version is unknown"}; package is v${PACKAGE_VERSION}.`, "Run `hermes-live setup` to install the matching plugin."));
    }
    ...
    if (config?.realtime.provider === "local") {
        ...
        if (isLoopbackHostname(endpoint.hostname)) {
            ...
            if (managedProfileSupported) {
                const memory = await inspectManagedLocalMemory(dependencies.totalMemoryBytes ?? totalmem(), dependencies.runner);
                checks.push(memory);
                ...
                if (!localService.installed) {
                    ...
                    checks.push(listening
                        ...
                        : fail("local-service", "Local voice service", `${localService.platform}: running, but the provider endpoint is not listening yet`, "Wait for the models to load. If it does not recover, run `hermes-live local logs`, then `hermes-live local restart`."));
                }
            }
        }
        else {
    ...
    checks.push(hermesCommand
        ...
        : warn("hermes-cli", "Hermes CLI", "The `hermes` command is not on PATH.", "Install Hermes or pass --hermes-command, then enable the plugin."));
    if (hermesCommand) {
    ...
    if (config) {
        ...
        checks.push(readiness.hermes.ok
            ...
            : fail("hermes-api", "Hermes API", hermesReadinessError, hermesReadinessError.startsWith("Hermes API Server is missing required ")
...
async function inspectHermesVersion(hermesCommand, runner) {
    ...
    if (result.code !== 0) {
        return warn("hermes-version", "Hermes version", result.timedOut ? "The version check timed out." : "The CLI did not report its version.", `Hermes Agent ${HERMES_COMPATIBILITY.minimumVersion} or newer is required.`);
    }
    const version = parseHermesVersion(`${result.stdout}\n${result.stderr}`);
    const status = classifyHermesVersion(version);
    if (status === "unsupported") {
        return fail("hermes-version", "Hermes version", `v${version} is unsupported.`, `Update Hermes Agent to v${HERMES_COMPATIBILITY.minimumVersion} or newer. v${HERMES_COMPATIBILITY.testedVersion} is the latest tested version.`);
    }
    if (status === "tested") {
        return pass("hermes-version", "Hermes version", `v${version} (${HERMES_COMPATIBILITY.testedReleaseTag}) is tested`);
    }
    if (status === "supported") {
        return pass("hermes-version", "Hermes version", `v${version} is supported; v${HERMES_COMPATIBILITY.testedVersion} is the latest tested version`);
    }
    if (status === "newer") {
        return warn("hermes-version", "Hermes version", `v${version} is newer than the latest tested version, v${HERMES_COMPATIBILITY.testedVersion}.`, "Keep the capability checks enabled and report any compatibility issue.");
    }
    return warn("hermes-version", "Hermes version", "The CLI output did not contain a semantic version.", `Hermes Agent ${HERMES_COMPATIBILITY.minimumVersion} or newer is required.`);
}
export function diagnoseManagedLocalMemory(totalMemoryBytes, memoryPressureOutput = "", swapUsageOutput = "") {
    ...
    if (!Number.isFinite(totalMemoryBytes) || totalMemoryBytes < MIN_MANAGED_LOCAL_MEMORY_BYTES) {
        return fail("local-memory", "Local voice memory", `${facts}. Managed local voice needs at least 12 GB.`, "Choose OpenAI or Gemini in `hermes-live setup`, or use a larger Apple Silicon host.");
    }
    if (totalMemoryBytes < RECOMMENDED_MANAGED_LOCAL_MEMORY_BYTES) {
        return warn("local-memory", "Local voice memory", `${facts}. 16 GB or more is recommended for consistent latency.`, "Close memory-heavy apps before using local voice, or choose a hosted voice provider.");
    }
    const highSwapThreshold = Math.max(HIGH_SWAP_MINIMUM_BYTES, totalMemoryBytes * 0.25);
    if ((parsedFreePercent !== undefined && parsedFreePercent < 10)
        || (swapUsedBytes !== undefined && swapUsedBytes >= highSwapThreshold)) {
        return warn("local-memory", "Local voice memory", `${facts}. Host memory pressure may make speech turns much slower.`, "Close memory-heavy apps, then run `hermes-live local restart` before judging local voice latency.");
    }
    return pass("local-memory", "Local voice memory", facts);
}
async function inspectManagedLocalMemory(totalMemoryBytes, runner) {
    ...
    return diagnoseManagedLocalMemory(totalMemoryBytes, pressure?.code === 0 ? pressure.stdout : "", swap?.code === 0 ? swap.stdout : "");
}
function parseMacSwapUsedBytes(value) {
    const match = /\bused\s*=\s*([0-9]+(?:\.[0-9]+)?)\s*([KMGT])\b/iu.exec(value);
    if (!match)
        return undefined;
    const amount = Number(match[1]);
    const exponent = { K: 1, M: 2, G: 3, T: 4 }[match[2].toUpperCase()];
...
function publicGatewayOrigin(config) {
    return gatewayOrigin(config.server.host, config.server.port);
}
async function pluginVersion(target) {
...
function shortHome(path, configuredHome) {
