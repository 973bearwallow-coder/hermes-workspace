...
export const MANAGED_LOCAL_ENTRYPOINT = fileURLToPath(new URL("../../assets/huggingface-realtime-entry.py", import.meta.url));
export async function runLocalVoiceCommand(args, config, dependencies = {}) {
    ...
    if (args.length > 1)
        throw new Error(`Unknown local voice option: ${args[1]}`);
    if (action === "logs") {
    ...
    if (action === "start" || action === "restart") {
        ...
        const installed = await runServiceAction("install", serviceOptions);
    ...
    console.log(`Starting Hugging Face speech-to-speech ${HUGGINGFACE_SPEECH_TO_SPEECH_VERSION} on ${publicLocalEndpoint(config.local.url)}.`);
    console.log("The first run downloads Python packages and model weights. Keep this terminal open; Ctrl+C stops local voice.");
    const code = await (dependencies.runForeground ?? runForeground)(command.command, command.args, { ...env, ...command.environment });
    if (code !== 0)
        throw new Error(`Hugging Face speech-to-speech exited with code ${code}.`);
}
export async function probeLocalVoiceEndpoint(url, timeoutMs = 350) {
    let endpoint;
    try {
...
export function localVoiceStartupProgress(logs) {
...
async function localVoiceServiceStatus(config, status, probe = probeLocalVoiceEndpoint) {
...
export async function resolveLocalVoiceCommand(config, dependencies = {}) {
    const env = dependencies.env ?? process.env;
    const uv = await (dependencies.findCommand ?? findExecutable)("uv", env);
    if (!uv) {
        throw new Error("uv is required for fully local voice. Install it with `brew install uv`, then rerun `hermes-live setup`.");
    }
    ...
    if (platform === "darwin"
        ...
        throw new Error("Managed local voice needs at least 12 GB of physical memory; a 16 GB Apple Silicon Mac is recommended. Choose OpenAI or Gemini in `hermes-live setup`, or use an external local voice endpoint.");
    }
...
export function buildLocalVoiceCommand(input) {
    if (input.platform !== "darwin" || input.arch !== "arm64") {
        throw new Error("The managed fully-local profile currently supports Apple Silicon. On other systems, run Hugging Face speech-to-speech in realtime mode and point HERMES_LIVE_LOCAL_URL at it.");
    }
...
export function printLocalVoiceHelp() {
...
function printServiceResult(action, result) {
...
function publicLocalEndpoint(value) {
...
function findMacCaBundle(env, platform) {
    if (platform !== "darwin")
        return undefined;
...
//# sourceMappingURL=local-voice.js.map