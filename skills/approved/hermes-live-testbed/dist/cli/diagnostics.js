...
const PACKAGE = packageRequire("../../package.json");
...
export function parseDiagnosticsOptions(args) {
    const remaining = args[0] === "export" ? args.slice(1) : args;
    const options = { providerSmoke: false };
    for (let index = 0; index < remaining.length; index += 1) {
        ...
        if (argument === "--output")
            ...
            throw new DiagnosticsHelpRequested();
        else if (argument)
            throw new Error(`Unknown diagnostics option: ${argument}`);
    }
    return options;
}
export async function runDiagnosticsCommand(args) {
    let options;
    try {
    ...
    if (!report.ok)
        console.log("The report contains one or more failed checks.");
}
export function createDiagnosticBundle(doctor, options = {}) {
    const raw = {
        schemaVersion: 1,
    ...
    return sanitizeDiagnosticValue(raw, options);
}
export function sanitizeDiagnosticValue(value, options = {}) {
    ...
    const visit = (current) => {
        ...
        if (options.home)
            sanitized = sanitized.replaceAll(options.home, "~");
        sanitized = sanitized
...
export async function writeDiagnosticBundle(path, bundle) {
...
function defaultDiagnosticFilename(generatedAt) {
