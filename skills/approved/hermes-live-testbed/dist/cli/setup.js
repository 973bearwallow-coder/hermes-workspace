...
import { isLoopbackHostname, loadConfig } from "../config.js";
import { LOCAL_FUNCTIONAL_PROVIDER_SMOKE_TIMEOUT_MS, runLiveProviderSmoke, } from "../live-provider-smoke.js";
import { buildReadinessReport } from "../readiness.js";
import { readManagedConfig, MANAGED_CONFIG_KEYS, writeManagedConfig, } from "./managed-config.js";
import { installHermesPlugin } from "./plugin-installer.js";
import { findExecutable, runCommand } from "./process.js";
import { runServiceAction } from "./service-manager.js";
import { localVoiceStartupProgress, probeLocalVoiceEndpoint, resolveLocalVoiceCommand, } from "./local-voice.js";
import { gatewayOrigin, probeGatewayEndpoint, probeGatewayReadiness, } from "./gateway-probe.js";
import { ensureHermesApiEnvironment, generateHermesApiKey, isDefaultLocalHermesApi, resolveHermesHome, } from "./hermes-api-bootstrap.js";
...
export function parseSetupOptions(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (argument === "--provider")
            options.provider = parseProvider(nextValue());
        else if (argument?.startsWith("--provider="))
            options.provider = parseProvider(argument.slice(11));
        else if (argument === "--hermes-url")
            options.hermesUrl = nextValue();
        else if (argument?.startsWith("--hermes-url="))
            options.hermesUrl = argument.slice(13);
        else if (argument === "--config")
            options.configPath = nextValue();
        else if (argument?.startsWith("--config="))
            options.configPath = argument.slice(9);
        else if (argument === "--plugins-dir")
            options.pluginsDir = nextValue();
        else if (argument?.startsWith("--plugins-dir="))
            options.pluginsDir = argument.slice(14);
        else if (argument === "--hermes-command")
            options.hermesCommand = nextValue();
        else if (argument?.startsWith("--hermes-command="))
            options.hermesCommand = argument.slice(17);
        else if (argument === "--no-enable")
            options.enablePlugin = false;
        else if (argument === "--no-service")
...
export async function runSetup(options, dependencies = {}) {
    ...
    if (provider === "gemini") {
        if (isEnabled(inherited.GOOGLE_GENAI_USE_ENTERPRISE)) {
            ...
            values.GOOGLE_CLOUD_PROJECT = await requireTextValue("Google Cloud project ID: ", inherited.GOOGLE_CLOUD_PROJECT, options, dependencies);
        }
        else {
            values.GEMINI_API_KEY = await requireSecret("Gemini API key: ", inherited.GEMINI_API_KEY ?? inherited.GOOGLE_API_KEY, options, dependencies);
        }
    }
    else if (provider === "openai") {
        values.OPENAI_API_KEY = await requireSecret("OpenAI API key: ", inherited.OPENAI_API_KEY, options, dependencies);
    }
    let preparedLocalVoiceCommand;
    ...
    if (provider === "local" && options.service && supportsManagedLocalService(dependencies)) {
        ...
        preparedLocalVoiceCommand = await resolveLocalVoiceCommand(loadConfig({ ...values, ...env }), dependencies);
    }
    ...
    if (provider === "local" && options.service && managedLocalProfile) {
        ...
        try {
            ...
            const installed = await runServiceAction("install", localServiceOptions);
            localService = installed.running
                ? installed
                : await runServiceAction("start", localServiceOptions);
        }
        catch (error) {
            localService = {
                skipped: true,
    ...
    let service = { skipped: true, reason: "Service installation was disabled." };
    let gateway = {
        checked: false,
        ready: false,
    ...
    if (options.service && activationPreflightOk) {
        ...
        const installedService = await runServiceAction("install", serviceOptions);
        service = installedService.running
            ? installedService
            : await runServiceAction("start", serviceOptions);
...
async function selectProvider(options, inherited, dependencies) {
    if (options.provider)
        return options.provider;
    const configured = inherited.HERMES_LIVE_PROVIDER;
    if (configured === "local" || configured === "gemini" || configured === "openai" || configured === "mock")
        return configured;
    if ((dependencies.platform ?? process.platform) === "darwin" && (dependencies.arch ?? process.arch) === "arm64")
        return "local";
    if (inherited.OPENAI_API_KEY && !inherited.GEMINI_API_KEY && !inherited.GOOGLE_API_KEY)
        return "openai";
    if (inherited.GEMINI_API_KEY || inherited.GOOGLE_API_KEY)
        return "gemini";
    ...
    if (!answer)
        throw new Error("Choose a voice provider, or rerun setup with --provider.");
    return parseProvider(answer);
}
async function requireSecret(prompt, existing, options, dependencies) {
    if (existing)
        return existing;
    if (options.nonInteractive) {
...
async function requireTextValue(prompt, existing, options, dependencies) {
    if (existing)
        return existing;
    if (options.nonInteractive) {
        throw new Error(`${prompt.replace(/:\s*$/u, "")} is required in non-interactive mode.`);
...
async function enableHermesPlugin(options, env, runner, findCommand, resolvedCommand) {
...
async function ensureHermesGatewayReady(config, input) {
    ...
    const commandResult = await input.runner(input.command, ["gateway", "install", "--force"], { env: input.env, timeoutMs: 120_000 });
...
function conciseCommandError(result) {
    if (result.timedOut)
        return "Hermes gateway installation timed out.";
    const detail = result.stderr.trim() || result.stdout.trim();
    if (!detail)
        return `Hermes gateway command exited with code ${result.code}.`;
    return detail.length > 500 ? `${detail.slice(0, 500)}…` : detail;
}
async function waitForGateway(config, dependencies) {
    ...
    while (Date.now() < deadline) {
        try {
            ...
            if (result.ok)
                return { checked: true, ready: true, url };
...
export async function resolveSetupGatewayPort(input) {
    ...
    if (state !== "occupied")
        return input.port;
    if (input.explicit) {
        throw new Error(`Gateway port ${input.port} is already used by another service. Choose a free HERMES_LIVE_PORT and rerun setup.`);
    }
    for (let offset = 1; offset <= MAX_GATEWAY_PORT_CANDIDATES; offset += 1) {
        const candidate = DEFAULT_GATEWAY_PORT + offset;
        if (candidate === input.port)
            continue;
    ...
    throw new Error(`Gateway port ${input.port} is already used by another service and no free port was found between ${DEFAULT_GATEWAY_PORT + 1} and ${DEFAULT_GATEWAY_PORT + MAX_GATEWAY_PORT_CANDIDATES}.`);
}
export async function resolveSetupLocalVoiceUrl(input) {
    ...
    if (!occupied || ownedByRunningService)
        return input.url;
    const port = endpoint.port ? Number(endpoint.port) : 80;
    if (input.explicit) {
        throw new Error(`Local voice port ${port} is already used by another process. Choose a free HERMES_LIVE_LOCAL_URL and rerun setup.`);
    }
    ...
    throw new Error(`Local voice port ${port} is already used by another process and no free port was found in the next ${MAX_LOCAL_VOICE_PORT_CANDIDATES} ports.`);
}
...
async function readLegacyHermesEnvironment(hermesHome) {
    ...
    if (stat.size > MAX_LEGACY_ENV_BYTES)
        throw new Error(`Hermes environment exceeds ${MAX_LEGACY_ENV_BYTES} bytes.`);
    const noFollow = "O_NOFOLLOW" in fsConstants ? fsConstants.O_NOFOLLOW : 0;
    const handle = await open(path, fsConstants.O_RDONLY | noFollow);
    try {
        const openedStat = await handle.stat();
        if (!openedStat.isFile())
            throw new Error(`Hermes environment must be a regular file: ${path}`);
...
function parseLegacyEnvironment(source) {
    ...
    for (const raw of source.split(/\r?\n/u)) {
        ...
        if (!accepted.has(key))
            continue;
        if (seen.has(key))
            throw new Error(`Hermes environment contains duplicate ${key}.`);
        result[key] = unquoteLegacyValue(normalized.slice(separator + 1).trim());
        seen.add(key);
    }
    return result;
}
async function readBoundedFile(handle, maximumBytes, path) {
    ...
    if (offset > maximumBytes)
        throw new Error(`Hermes environment exceeds ${maximumBytes} bytes: ${path}`);
    return buffer.subarray(0, offset).toString("utf8");
}
function unquoteLegacyValue(value) {
...
function firstDefinedEnvironment(env, managed, legacy) {
    const keys = new Set([...Object.keys(legacy), ...Object.keys(managed), ...Object.keys(env)]);
    return Object.fromEntries([...keys].map((key) => [key, env[key] || managed[key] || legacy[key]]));
}
...
function isHermesApiCompatibilityError(message) {
    return message.startsWith("Hermes API Server is missing required ");
}
function parseProvider(value) {
    if (value === "local" || value === "gemini" || value === "openai" || value === "mock")
        return value;
    throw new Error(`Unsupported provider: ${value}. Choose local, gemini, openai, or mock.`);
}
async function checkProviderSession(config, options = {}) {
...
async function waitForProviderSession(config, dependencies) {
    ...
    const check = dependencies.providerSessionCheck
        ?? ((candidate) => checkProviderSession(candidate, {
            verifyToolCall: true,
            timeoutMs: Math.max(candidate.server.providerReadyTimeoutMs, LOCAL_FUNCTIONAL_PROVIDER_SMOKE_TIMEOUT_MS),
        }));
...
async function localProviderProgress(dependencies) {
    if (dependencies.localProviderProgress)
        return await dependencies.localProviderProgress();
    try {
...
function isManagedLocalProfile(config, dependencies) {
...
function supportsManagedLocalService(dependencies) {
    return (dependencies.platform ?? process.platform) === "darwin"
        && (dependencies.arch ?? process.arch) === "arm64";
}
function managedValuesFromEnvironment(values) {
    const managed = {};
