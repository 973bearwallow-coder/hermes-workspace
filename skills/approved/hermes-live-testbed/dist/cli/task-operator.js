...
import { FileTaskStore, TaskStoreLockedError, clearAbandonedTaskStoreLock, } from "../adapters/outbound/task-store/file-task-store.js";
import { TaskIdSchema, containIndeterminateTask, } from "../domain/tasks/index.js";
export async function runOfflineTaskCommand(args, config, write = console.log) {
    ...
    if (command === "unlock") {
        if (args.length !== 2 || args[1] !== "--confirm-no-gateway") {
            throw new Error("Usage: hermes-live tasks unlock --confirm-no-gateway. " +
    ...
    try {
        if (command === "unresolved") {
            if (args.length !== 1)
                throw new Error("Usage: hermes-live tasks unresolved");
        ...
        if (command === "contain") {
            if (args.length !== 3 || args[2] !== "--confirm-contained") {
                throw new Error("Usage: hermes-live tasks contain <taskId> --confirm-contained. " +
            ...
            const contained = await store.update(taskId, (record) => containIndeterminateTask(record), { expectedRevision: current.revision });
        ...
        commandFailure = error instanceof TaskStoreLockedError
            ? new Error("The task state is in use. Stop the running Hermes Live gateway before using offline task recovery.", { cause: error })
            : error;
        ...
        try {
            ...
            if (commandFailed) {
                throw new AggregateError([commandFailure, cleanupError], "The offline task command failed and task-state cleanup also failed.");
            }
            throw cleanupError;
        }
    }
}
export function taskCommandHelp() {
    return [
...
function isOperationallyUnresolved(record) {
...
function operatorTaskSummary(record) {
    return {
