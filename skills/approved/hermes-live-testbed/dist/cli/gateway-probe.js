...
export function gatewayOrigin(host, port) {
...
export async function probeGatewayEndpoint(host, port, options = {}) {
...
export async function probeGatewayReadiness(origin, options = {}) {
    try {
...
async function probeGatewayHealthIdentity(origin, fetchImplementation, timeoutMs = DEFAULT_PROBE_TIMEOUT_MS) {
    try {
...
export async function readBoundedGatewayJson(response) {
    ...
    if (!response.body)
        throw new Error("Gateway probe response is empty.");
...
async function probeTcpEndpoint(host, port, timeoutMs) {
...
//# sourceMappingURL=gateway-probe.js.map