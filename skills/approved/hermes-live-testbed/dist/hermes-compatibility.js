...
export function classifyHermesVersion(version) {
    ...
    if (!current || !minimum || !tested)
        return "unknown";
    if (compareSemanticVersions(current, minimum) < 0)
        return "unsupported";
    const comparedWithTested = compareSemanticVersions(current, tested);
    if (comparedWithTested === 0)
        return "tested";
    if (comparedWithTested > 0)
        return "newer";
    return "supported";
...
//# sourceMappingURL=hermes-compatibility.js.map