...
import { buildLocalConversationResponse, buildLocalExactSpeechResponse, buildLocalTaskQuestionResponse, isClearLocalWorkRequest, localRoutedAction, matchLocalStoppableTask, selectLocalFinishedTask, selectLocalStoppableTasks, selectLocalTaskForQuestion, } from "./huggingface-local-routing.js";
import { buildOpenAITaskNotificationResponse, normalizeOpenAIRealtimeEvent, } from "./openai-realtime.adapter.js";
...
const LOCAL_TURN_END_SILENCE = Buffer.alloc(LOCAL_REALTIME_PCM_SAMPLE_RATE * 2 * LOCAL_TURN_END_SILENCE_MS / 1_000).toString("base64");
 ...
 */
export class HuggingFaceRealtimeAdapter {
    config;
    connectTimeoutMs;
    closeTimeoutMs;
    responseTimeoutMs;
    pipelineReleaseGraceMs;
    constructor(config, connectTimeoutMs = DEFAULT_CONNECT_TIMEOUT_MS, closeTimeoutMs = DEFAULT_CLOSE_TIMEOUT_MS, responseTimeoutMs = DEFAULT_LOCAL_RESPONSE_TIMEOUT_MS, pipelineReleaseGraceMs = DEFAULT_LOCAL_PIPELINE_RELEASE_GRACE_MS) {
        this.config = config;
        this.connectTimeoutMs = connectTimeoutMs;
        this.closeTimeoutMs = closeTimeoutMs;
        this.responseTimeoutMs = responseTimeoutMs;
        this.pipelineReleaseGraceMs = pipelineReleaseGraceMs;
    }
    async connect(params) {
        ...
        for (;;) {
            ...
            if (remainingMs <= 0) {
                throw new Error(`Hugging Face speech-to-speech stayed busy for ${this.connectTimeoutMs}ms.`);
            }
            try {
                return await this.connectOnce(params, remainingMs);
            }
            catch (error) {
                if (!this.config.ownsTurnRouting
    ...
    async connectOnce(params, timeoutMs) {
        ...
        const session = new HuggingFaceRealtimeSession(ws, this.config, params.callbacks, this.closeTimeoutMs, this.responseTimeoutMs, this.pipelineReleaseGraceMs);
        return await new Promise((resolve, reject) => {
            let settled = false;
            let configured = false;
            let configurationSettle;
            const timeout = setTimeout(() => {
                fail(new Error(`Hugging Face speech-to-speech did not create a realtime session within ${timeoutMs}ms.`));
            }, timeoutMs);
            timeout.unref?.();
            const cleanup = () => {
                clearTimeout(timeout);
                if (configurationSettle)
                    clearTimeout(configurationSettle);
                ws.off("error", fail);
            ...
            const fail = (error) => {
                ...
                if (ws.readyState === WebSocket.OPEN)
                    ws.close(1011, "Local voice session start failed");
                else if (ws.readyState === WebSocket.CONNECTING)
                    ws.terminate();
                reject(failure);
            };
            const onClose = (code, reason) => {
                fail(new Error(`Hugging Face speech-to-speech closed before session start: ${code} ${reason.toString("utf8")}`));
            };
...
class HuggingFaceRealtimeSession {
    ws;
    config;
    callbacks;
    closeTimeoutMs;
    ...
    syntheticToolCalls = new Map();
    availableTools;
    responsePending = false;
    responseActive = false;
    implicitResponsePending = false;
    inputSpeechActive = false;
    pendingCompletedVoiceTranscript;
    referencedTaskId;
    audioBuffered = false;
    activeResponseId;
    pendingResponseKind;
    activeResponseKind;
    responseProducedOutput = false;
    responseProducedToolCall = false;
    bufferedAssistantText = [];
    bufferedAssistantTextChars = 0;
    responseTimeout;
    providerClosedAt;
    closing = false;
    ready = false;
    closeOperation;
    constructor(ws, config, callbacks, closeTimeoutMs, responseTimeoutMs, pipelineReleaseGraceMs) {
        this.ws = ws;
        this.config = config;
    ...
    async cancelResponse(_reason, _truncate) {
    ...
    async sendToolResponse(call, response) {
        if (!call.id)
            throw new Error(`Hugging Face function call ${call.name} did not include a call_id.`);
        const spokenReceipt = localSpokenToolReceipt(response);
        const returnedTaskId = localReturnedTaskId(response);
        if (returnedTaskId)
            this.referencedTaskId = returnedTaskId;
            ...
            if (spokenReceipt) {
                ...
                if (stoppable.length === 0) {
                    ...
                    if (match.status === "matched") {
                        ...
                        this.schedule({
                            kind: "conversation",
                            response: buildLocalExactSpeechResponse(match.status === "ambiguous"
                    ...
                    this.schedule({
                        kind: "conversation",
                        response: buildLocalExactSpeechResponse(`You have ${stoppable.length} active tasks. Say which one you want me to stop.`),
                    });
                }
                else {
                ...
                if (!parentTaskId) {
                    this.schedule({
                        kind: "conversation",
                        response: buildLocalExactSpeechResponse("I couldn't find a finished background task to follow up."),
                    });
                }
                else {
                    const followUp = {
                    ...
                    if (this.canRouteAction(followUp)) {
                        ...
                        this.schedule({
                            kind: "conversation",
                            response: buildLocalExactSpeechResponse("This Hermes version can't start task follow-ups."),
                        });
                    }
                }
            }
    ...
    close() {
        ...
        if (this.ws.readyState === WebSocket.CLOSED) {
            this.closeOperation = waitForLocalPipelineRelease(this.providerClosedAt ?? Date.now(), this.pipelineReleaseGraceMs);
            return this.closeOperation;
        }
        this.closing = true;
        this.reset();
        this.closeOperation = closeLocalSocket(this.ws, this.closeTimeoutMs, this.pipelineReleaseGraceMs);
        return this.closeOperation;
    }
    handleMessage(raw) {
        if (this.closing)
        ...
        const toolCalls = normalized.filter((value) => value.type === "tool_call");
        ...
        for (const original of normalized) {
            ...
            if (isUsableLocalResponseOutput(value))
                this.responseProducedOutput = true;
            if (isAssistantText(value)) {
                if (!this.responseProducedToolCall && !this.bufferAssistantText(value))
                    return;
                continue;
            }
            if (value.type === "audio" && this.responseProducedToolCall)
                continue;
            if (terminal && value.type === "response") {
                if (!this.responseProducedToolCall)
                    deliverable.push(...this.takeBufferedAssistantText());
            }
            deliverable.push(value);
        }
        for (const value of deliverable)
        ...
        if (this.config.ownsTurnRouting && finalVoiceTranscript) {
            if (this.implicitResponsePending) {
                if (!this.routeManagedVoiceTranscript(finalVoiceTranscript))
                    return;
            }
    ...
    schedule(request) {
        if (this.closing)
            throw new Error("Local voice session is closing.");
        if (this.busy()) {
    ...
    sendJson(value) {
        if (this.ws.readyState !== WebSocket.OPEN)
            throw new Error("Local voice WebSocket is not open.");
    ...
    fail(error) {
        ...
        this.reset();
        if (this.ws.readyState === WebSocket.OPEN)
            this.ws.close(1011, "Local voice provider error");
    ...
    armResponseTimeout() {
        ...
        this.responseTimeout = setTimeout(() => {
            ...
            if (this.closing || (!this.responsePending && !this.responseActive && !this.implicitResponsePending))
                return;
    ...
    canRouteAction(action) {
        return this.availableTools === undefined || this.availableTools.has(action.name);
    }
    routedAction(text) {
        const explicit = localRoutedAction(text);
        if (explicit)
            return this.canRouteAction(explicit) ? explicit : undefined;
        const referencedStop = this.referencedTaskId
            ...
            : undefined;
        if (referencedStop && this.canRouteAction(referencedStop))
    ...
    routeManagedVoiceTranscript(transcript) {
        this.pendingCompletedVoiceTranscript = undefined;
        this.implicitResponsePending = false;
        this.clearResponseTimeout();
    ...
    bufferAssistantText(event) {
        const nextChars = this.bufferedAssistantTextChars + event.text.length;
        if (this.bufferedAssistantText.length >= LOCAL_MAX_BUFFERED_ASSISTANT_TEXT_EVENTS
    ...
    dropBufferedAssistantText() {
        this.bufferedAssistantText.length = 0;
        this.bufferedAssistantTextChars = 0;
    }
    clearBufferedResponseOutput() {
        this.responseProducedToolCall = false;
        this.dropBufferedAssistantText();
    }
}
export function buildHuggingFaceSessionUpdate(config, systemInstruction, availableTools) {
...
function localProviderError(event) {
...
function isUsableLocalResponseOutput(event) {
...
function localResponseStatusReason(event) {
    const reason = event?.response?.status_details?.reason;
    return typeof reason === "string" ? reason : undefined;
}
function localCompletedUserTranscript(event) {
...
function closeLocalSocket(ws, timeoutMs, releaseGraceMs) {
    ...
    return new Promise((resolve, reject) => {
        ...
        ws.once("close", () => {
            ...
            if (releaseGraceMs <= 0)
                ...
                setTimeout(resolve, releaseGraceMs);
        });
        if (ws.readyState === WebSocket.OPEN)
...
function waitForLocalPipelineRelease(closedAt, releaseGraceMs) {
    const remainingMs = Math.max(0, releaseGraceMs - (Date.now() - closedAt));
    return remainingMs === 0
        ? Promise.resolve()
        : new Promise((resolve) => setTimeout(resolve, remainingMs));
}
//# sourceMappingURL=huggingface-realtime.adapter.js.map