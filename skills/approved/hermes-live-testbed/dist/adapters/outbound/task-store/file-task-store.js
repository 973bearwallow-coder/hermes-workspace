...
import { z } from "zod";
import { MAX_TASK_EVENTS, TASK_RECORD_SCHEMA_VERSION, TaskIdSchema, TaskOwnerIdSchema, TaskRecordSchema, TaskStatusSchema, canTransitionTask, isTaskTerminal, parseTaskRecord, parseTaskTimestamp, } from "../../../domain/tasks/index.js";
...
export class FileTaskStore {
    ...
    constructor(options) {
        ...
        this.terminalReserveSlots = boundedPositiveInteger(options.terminalReserveSlots ?? 1, "terminalReserveSlots", MAX_CONFIGURED_TERMINAL_RESERVE_SLOTS);
    ...
    list(options = {}) {
        ...
        return this.serialized(async () => {
            await this.ensureLoaded();
            const statusSet = options.statuses ? new Set(options.statuses.map((value) => TaskStatusSchema.parse(value))) : undefined;
    ...
    put(value) {
        return this.serialized(async () => {
            ...
            if (next.size >= this.maxRecords) {
                throw new TaskStoreCapacityError(`Task store reached its ${this.maxRecords}-record capacity and has no operationally closed record eligible for pruning.`);
            }
    ...
    update(taskId, updater, options = {}) {
        ...
        return this.serialized(async () => {
            ...
            if (options.expectedRevision !== undefined && current.revision !== options.expectedRevision) {
                throw new TaskStoreConflictError(`Task revision conflict for ${id}: expected ${options.expectedRevision}, found ${current.revision}.`);
            }
            ...
            if (current.operatorContainedAt !== undefined
            ...
            if (current.operatorContainedAt === undefined && updated.operatorContainedAt !== undefined) {
                ...
                if (containmentEvent?.type !== "operator_contained"
            ...
            if (current.upstreamRunMissingAt !== undefined
            ...
            if (current.upstreamRunMissingAt === undefined && updated.upstreamRunMissingAt !== undefined) {
                ...
                if (missingEvent?.type !== "unknown" || missingEvent.timestamp !== updated.upstreamRunMissingAt) {
                    throw new TaskStoreConflictError("A confirmed missing upstream run requires its exact append-only unknown event.");
                }
            }
    ...
    prune(options = {}) {
        return this.serialized(async () => {
            await this.ensureLoaded();
            const terminalBefore = parseTaskTimestamp(options.terminalBefore ?? Math.max(0, this.now() - this.retentionMs), "Prune timestamp");
            ...
            try {
                ...
                if (!this.records && this.lockHeld) {
                    try {
                        ...
                        throw new AggregateError([error, cleanupError], "Task store initialization failed and its writer lock could not be released cleanly.");
                    }
                }
                throw error;
            }
    ...
    async ensureLoaded() {
        ...
        try {
            ...
            if (stat.size > this.maximumStoreBytes) {
                throw new TaskStoreCorruptionError(`Task store exceeds its ${this.maximumStoreBytes}-byte bounded migration safety limit.`);
            }
        ...
        if (!parsed.success) {
            throw new TaskStoreCorruptionError(`Task store failed schema validation; refusing to reset it: ${parsed.error.issues[0]?.message ?? "invalid document"}`);
        }
        const loaded = new Map(parsed.data.tasks.map((task) => [task.taskId, cloneTask(task)]));
        let pruned = [];
        if (this.automaticPruning) {
            pruned = this.pruneMap(loaded, Math.max(0, this.now() - this.retentionMs), this.maxRecords);
            if (loaded.size > this.maxRecords) {
                throw new TaskStoreCorruptionError(`Task store has ${loaded.size} non-prunable records, above the configured ${this.maxRecords}-record limit.`);
            }
        }
        ...
        if (this.automaticPruning) {
            ...
            try {
                this.fitDocumentToByteLimit(normalCandidate, parsed.data.updatedAt, new Set(), this.maxStoreBytes);
                selected = normalCandidate;
                this.capacityLimitBytes = this.maxStoreBytes;
            }
            catch (error) {
                if (!(error instanceof TaskStoreCapacityError))
                    throw error;
                ...
                this.fitDocumentToByteLimit(migrationCandidate, parsed.data.updatedAt, new Set(), this.maximumStoreBytes);
            ...
            this.capacityLimitBytes = this.maximumStoreBytes;
            this.fitDocumentToByteLimit(selected, parsed.data.updatedAt, new Set(), this.maximumStoreBytes);
        }
    ...
    async persist(records, options = {}) {
        await this.ensureDirectory();
        await this.assertLockOwned();
        const updatedAt = Math.max(this.documentUpdatedAt, parseTaskTimestamp(this.now(), "Store timestamp"));
        const { document, payload } = this.fitDocumentToByteLimit(records, updatedAt, options.protectedTaskIds ?? new Set(), options.capacityLimitBytes ?? this.capacityLimitBytes, options.reserveQueuedCapacity !== false, options.reserveControlCapacity !== false);
        ...
        try {
            ...
            if (renamed) {
                ...
                this.poisoned = new TaskStoreCorruptionError("Task store commit could not be confirmed after rename; restart the gateway before retrying.", { cause: error });
                throw this.poisoned;
            }
            throw error;
        }
    ...
    fitDocumentToByteLimit(records, updatedAt, protectedTaskIds, capacityLimitBytes = this.capacityLimitBytes, reserveQueuedCapacity = true, reserveControlCapacity = true) {
        ...
        const reserveBytes = this.automaticPruning
            ? capacityReserveBytes(records, this.terminalReserveSlots, reserveQueuedCapacity, reserveControlCapacity)
            : 0;
        const payloadLimit = limit - reserveBytes;
        if (payloadLimit <= 0) {
            throw new TaskStoreCapacityError(`Task store cannot preserve its ${reserveBytes}-byte transition reserve within the ${limit}-byte limit.`);
        }
        ...
        if (bytes <= payloadLimit)
            return { document: built, payload };
        if (!this.automaticPruning) {
            throw new TaskStoreCapacityError(`Task store write exceeds its ${limit}-byte safety limit; automatic history pruning is disabled.`);
        }
        const candidates = [...records.values()]
            .filter((record) => !protectedTaskIds.has(record.taskId)
        ...
        if (bytes <= payloadLimit)
            return { document: built, payload };
        throw new TaskStoreCapacityError(`Task store cannot preserve ${reserveBytes} bytes for durable transitions within its ` +
            `${limit}-byte safety limit after pruning eligible history.`);
    }
    pruneMap(records, terminalBefore, maxRecords) {
        const deleted = [];
    ...
    async acquireLock() {
        ...
        try {
            ...
            if (isNodeError(error, "EEXIST")) {
                throw new TaskStoreLockedError(`Task state is already owned by another gateway: ${this.filePath}. ` +
        ...
        try {
            ...
            handle = await open(this.lockOwnerPath, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY | constants.O_NOFOLLOW, 0o600);
            await handle.writeFile(payload, "utf8");
            await handle.sync();
            await handle.close();
            handle = undefined;
    ...
    async assertLockOwned() {
        if (this.poisoned)
            throw this.poisoned;
        if (!this.lockHeld)
            throw new TaskStoreLockedError("Task store ownership has not been acquired.");
        try {
            ...
            this.poisoned = error instanceof TaskStoreCorruptionError
                ? error
                : new TaskStoreCorruptionError("Task store ownership could not be verified; restart the gateway before retrying.", { cause: error });
            throw this.poisoned;
        }
    }
    async releaseLock() {
        if (!this.lockHeld) {
            if (this.lockCleanupFailure)
        ...
        try {
            ...
            if (ownerRemoved) {
                ...
                this.lockHeld = false;
                this.lockCleanupFailure = new TaskStoreCorruptionError("Task store writer lock was only partially released; inspect the lock directory before restarting.", { cause: error });
                throw this.lockCleanupFailure;
            }
            throw error;
        }
 ...
 */
export async function clearAbandonedTaskStoreLock(options) {
    ...
    try {
        ...
        if (!(error instanceof TaskStoreCorruptionError))
            throw error;
    ...
    if (owner?.hostname === hostname() && isProcessAlive(owner.pid)) {
        throw new TaskStoreLockedError(`Refusing to clear task state owned by live gateway PID ${owner.pid} on ${owner.hostname}.`);
    }
    ...
    try {
        ...
        if (isNodeError(error, "ENOTEMPTY") || isNodeError(error, "EEXIST")) {
            throw new TaskStoreCorruptionError(`Task store lock contains unexpected files; refusing to remove it: ${lockDirectory}`, { cause: error });
        }
        if (!isNodeError(error, "ENOENT"))
            throw error;
    }
...
function buildTaskStoreDocument(records, updatedAt) {
    return {
...
function hasSameTaskDefinition(current, updated) {
...
function comparePrunableRecords(left, right) {
...
function capacityReserveBytes(records, maximumConcurrentSlots, reserveQueuedCapacity, reserveControlCapacity) {
    const maximum = boundedPositiveInteger(maximumConcurrentSlots, "maximum concurrent terminal reserve slots", MAX_CONFIGURED_TERMINAL_RESERVE_SLOTS);
    let active = 0;
    let queued = 0;
...
function isTaskOperationallyClosedForCapacity(record) {
    return isTaskTerminal(record.status)
        || record.upstreamRunMissingAt !== undefined
        || record.operatorContainedAt !== undefined;
}
function cloneTaskMap(records) {
...
async function readLockOwner(ownerPath, options = {}) {
    let handle;
    try {
    ...
    try {
        const stat = await handle.stat();
        if (!stat.isFile())
            throw new TaskStoreCorruptionError("Task store lock owner is not a regular file.");
        ...
        if (!owner.success) {
            throw new TaskStoreCorruptionError(`Task store lock owner failed validation: ${owner.error.issues[0]?.message ?? "invalid document"}`);
        }
        return owner.data;
    }
    finally {
...
//# sourceMappingURL=file-task-store.js.map