...
const GoogleCloudProjectSchema = z.preprocess((value) => value === "" ? undefined : value, z.string().min(6).max(30).refine(isSafeGoogleCloudProject, {
...
const GoogleGenAiApiVersionSchema = z.preprocess((value) => value === "" ? undefined : value, z.string().min(2).max(32).refine(isSafeGoogleGenAiApiVersion, {
    message: "GOOGLE_GENAI_API_VERSION must be a bounded v1/v1beta/v1alpha-style token.",
}).optional());
const TaskStateFileSchema = z.string().min(1).max(MAX_STATE_FILE_PATH_CHARS).refine((value) => isAbsolute(value) && value === value.trim() && !/[\u0000-\u001f\u007f]/u.test(value), { message: "HERMES_LIVE_TASK_STATE_FILE must be a bounded absolute path." });
const OpenAITranscriptionModelSchema = z.string().trim().min(1).max(128).refine((value) => value === "disabled" || /^[a-z0-9][a-z0-9._-]*$/u.test(value), { message: "OPENAI_REALTIME_INPUT_TRANSCRIPTION_MODEL must be a model id or disabled." });
const OpenAITranscriptionLanguageSchema = z.preprocess((value) => value === "" ? undefined : value, z.string().regex(/^[a-z]{2}$/u, {
...
const EnvSchema = z.object({
    ...
    HERMES_LIVE_TASK_STATE_FILE: TaskStateFileSchema.default(join(homedir(), ".hermes", "hermes-live", "tasks-v1.json")),
...
export function loadConfig(env = process.env) {
    ...
    return {
        server: {
            ...
            demoEnabled: parsed.HERMES_LIVE_DEMO_ENABLED === undefined
...
export function assertRealtimeProviderConfig(config) {
    if (config.realtime.provider === "local") {
        ...
        if (!isLoopbackHostname(endpoint.hostname) && !config.local.allowRemote) {
            throw new Error("HERMES_LIVE_LOCAL_URL must use localhost by default. Set HERMES_LIVE_LOCAL_ALLOW_REMOTE=true only for a trusted Hugging Face speech-to-speech endpoint.");
        }
    ...
    if (config.realtime.provider === "gemini" && !config.gemini.enterprise && !config.gemini.apiKey) {
        throw new Error("Set GEMINI_API_KEY or GOOGLE_API_KEY, enable GOOGLE_GENAI_USE_ENTERPRISE=true, or use HERMES_LIVE_PROVIDER=mock for local text-only development.");
    }
}
export function realtimeProviderConfigured(config) {
    if (config.realtime.provider === "local") {
        return true;
    }
...
export function sanitizeSessionComponent(value) {
    const sanitized = [];
...
function isSafeSessionCharacter(character) {
...
export function makeSessionKey(prefix, profileId, userLabel) {
...
function isSafeHermesBaseUrl(value) {
...
function isSafeOpenAIRealtimeBaseUrl(value) {
    return isSafeRealtimeWebSocketUrl(value);
}
function isSafeRealtimeWebSocketUrl(value) {
    const parsed = parseSafeConfiguredUrl(value);
    return Boolean(parsed &&
        ["ws:", "wss:"].includes(parsed.protocol) &&
        !value.includes("#"));
}
export function isSafeGoogleCloudProject(value) {
    return /^[a-z][a-z0-9-]{4,28}[a-z0-9]$/u.test(value);
}
export function isSafeGoogleCloudLocation(value) {
    return value.length <= 63 && /^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(value);
}
export function isSafeGoogleGenAiApiVersion(value) {
    return value.length <= 32 && /^v[1-9][0-9]*(?:(?:alpha|beta)[0-9]*)?$/u.test(value);
}
function parseSafeConfiguredUrl(value) {
    if (!value ||
...
export function publicBaseUrl(value) {
    ...
    try {
        ...
        if (!parsed.hostname || origin.length > 512)
            return "[invalid-url]";
...
function selectedRealtimeModel(provider, geminiModel, openaiModel) {
    if (provider === "local") {
...
function isNetworkAccessibleHost(host) {
...
function isPrivateNetworkHostname(hostname) {
    ...
    if (value === "host.docker.internal" || value.endsWith(".internal") || value.endsWith(".local"))
        return true;
