...
import { installHermesPlugin, pluginInstallStatus, pluginSourceDir, } from "./cli/plugin-installer.js";
import { applyManagedConfigToProcess } from "./cli/managed-config.js";
import { runServiceAction } from "./cli/service-manager.js";
...
const logger = createLogger(process.env.HERMES_LIVE_LOG_LEVEL ?? "info");
...
async function main() {
    ...
    if (printRequestedCommandHelp(command, commandArgs))
        return;
...
function usesManagedRuntimeConfig(command) {
    return [
        "serve",
        "dev",
        "client",
...
function printRequestedCommandHelp(command, args) {
...
function parseTerminalConversation(args) {
    if (args.length === 0 || (args.length === 1 && args[0] === "--new")) {
        return { mode: "new" };
    }
    if (args.length === 1 && args[0] === "--unbound")
        return { mode: "unbound" };
...
function parsePluginOptions(args) {
    const options = { mode: "copy", force: false };
...
async function runTextClient(config, text) {
    ...
    const readyTimeoutMs = positiveInt(process.env.HERMES_LIVE_CLIENT_READY_TIMEOUT_MS, DEFAULT_TEXT_CLIENT_READY_TIMEOUT_MS);
    const resultTimeoutMs = positiveInt(process.env.HERMES_LIVE_CLIENT_RESULT_TIMEOUT_MS, DEFAULT_TEXT_CLIENT_RESULT_TIMEOUT_MS);
    ...
    await new Promise((resolve, reject) => {
        ...
        const clearTimers = () => {
            ...
            if (state.directCompletionTimer)
                clearTimeout(state.directCompletionTimer);
            readyTimeout = undefined;
            resultTimeout = undefined;
            state.directCompletionTimer = undefined;
        };
        const finish = (error, terminate = false) => {
            if (state.finished)
                return;
            state.finished = true;
            clearTimers();
        ...
        const scheduleDirectCompletion = () => {
            if (state.directCompletionTimer || state.boundTaskId)
                return;
        ...
        readyTimeout = setTimeout(() => {
            finish(new Error(`Gateway did not complete the WebSocket/session.ready handshake within ${readyTimeoutMs}ms.`), true);
        }, readyTimeoutMs);
        readyTimeout.unref?.();
        ws.once("open", () => {
            if (state.finished)
...
async function handleClientServerMessage(ws, raw, text, state, markSessionReady, scheduleDirectCompletion, finish) {
    if (state.finished)
        return;
    ...
    switch (message.type) {
        case "session.ready": {
            if (state.sessionReady)
                throw new Error("Gateway sent duplicate session.ready messages.");
            ...
            if (protocolVersion !== HERMES_LIVE_PROTOCOL_VERSION) {
                throw new Error(`Gateway protocol mismatch: expected v${HERMES_LIVE_PROTOCOL_VERSION}, received ${String(protocolVersion)}.`);
            }
        ...
        case "task.accepted": {
            ...
            if (!state.boundTaskId && state.knownTaskIds.has(taskId))
                break;
            if (!state.boundTaskId) {
                ...
                console.error(`Additional Hermes task accepted: ${sanitizeTerminalText(taskId, MAX_TEXT_CLIENT_ID_CHARS)}; waiting for ${sanitizeTerminalText(state.boundTaskId, MAX_TEXT_CLIENT_ID_CHARS)}.`);
            }
            break;
        }
        ...
        case "response.cancelled":
            if (!state.boundTaskId)
                finish(new Error("Realtime provider response was cancelled."));
            break;
        case "task.started":
        ...
        case "task.snapshot": {
            if (message.reason === "initial" || message.reason === "reconnect") {
                for (const task of message.tasks)
                    state.knownTaskIds.add(task.taskId);
                break;
            }
            if (!state.resultRequestId || message.requestId !== state.resultRequestId)
                break;
            if (message.reason !== "get")
                throw new Error("Gateway task snapshot did not answer the one-shot task.get request.");
        ...
        case "task.failed":
            if (message.taskId === state.boundTaskId)
                finish(new Error(`Hermes task failed: ${message.error.message}`));
            break;
        case "task.cancelled":
            if (message.taskId === state.boundTaskId) {
        ...
        case "task.unknown":
            if (message.taskId === state.boundTaskId)
                finish(new Error(`Hermes task outcome is unknown: ${message.error.message}`));
            break;
        case "session.error":
...
function finishDirectResponse(state, finish) {
...
function parseTextClientServerMessage(raw) {
    let value;
    try {
    ...
    if (message.type === "session.ready" && message.protocolVersion !== HERMES_LIVE_PROTOCOL_VERSION) {
        throw new Error(`Gateway protocol mismatch: expected v${HERMES_LIVE_PROTOCOL_VERSION}, received ${String(message.protocolVersion)}.`);
    }
    try {
...
function requiredString(message, field, type, maxChars, allowEmpty = false) {
...
function requiredInteger(message, field, type) {
    const value = message[field];
    if (!Number.isSafeInteger(value))
        throw new Error(`Gateway ${type} ${field} must be an integer.`);
    return value;
}
...
function sendTextClientMessage(ws, message) {
    if (ws.readyState !== WebSocket.OPEN)
        throw new Error("Gateway WebSocket is not open.");
    ws.send(JSON.stringify(message));
}
function textClientServerMessageBytes(config) {
...
function printOneShotOutput(output) {
    console.log(sanitizeTerminalText(output, MAX_TEXT_CLIENT_OUTPUT_CHARS));
}
function finishFromTaskSnapshot(task, finish) {
...
function defaultGatewayWebSocketUrl(config) {
