...
export function normalizePcm16Audio(audio, targetRate) {
...
export function requirePcmSampleRate(mimeType) {
    const sampleRate = parsePcmSampleRate(mimeType);
    if (sampleRate === undefined) {
        throw new Error(`PCM audio mime type must include exactly one integer rate between ${MIN_PCM_SAMPLE_RATE} and ${MAX_PCM_SAMPLE_RATE} Hz.`);
    }
    return sampleRate;
}
export function isValidPcmSampleRate(sampleRate) {
    return (typeof sampleRate === "number" &&
        Number.isInteger(sampleRate) &&
        sampleRate >= MIN_PCM_SAMPLE_RATE &&
        sampleRate <= MAX_PCM_SAMPLE_RATE);
}
export function validatePcmSampleRate(sampleRate, label = "PCM sample rate") {
...
export function pcmMimeType(sampleRate) {
    validatePcmSampleRate(sampleRate);
    return `audio/pcm;rate=${sampleRate}`;
}
export function resamplePcm16Base64(data, sourceRate, targetRate) {
...
function decodePcm16Base64(data) {
