...
export const TaskIdSchema = z.string().regex(/^task_[0-9a-f]{32}$/u, "Task ID must use the task_<32 lowercase hex> format.");
...
export const TaskHermesSessionIdSchema = z.string().min(1).max(256).refine(isSafeSingleLine, "Hermes session ID contains unsafe characters.");
const TaskTitleSchema = z.string().min(1).max(MAX_TASK_TITLE_CHARS).refine(isSafeSingleLine, "Task title contains unsafe characters.");
const TaskInputSchema = z.string().min(1).max(MAX_TASK_INPUT_CHARS).refine(isSafeMultiline, "Task input contains unsafe control characters.");
const TaskOutputSchema = z.string().max(MAX_TASK_OUTPUT_CHARS).refine(isSafeMultiline, "Task output contains unsafe control characters.");
const TaskErrorSchema = z.string().min(1).max(MAX_TASK_ERROR_CHARS).refine(isSafeMultiline, "Task error contains unsafe control characters.");
export const TaskResourceKeySchema = z.string().min(1).max(MAX_TASK_RESOURCE_KEY_CHARS).refine(isSafeSingleLine, "Task resource key contains unsafe characters.");
...
export const TaskEventSchema = z.object({
    ...
    summary: z.string().min(1).max(MAX_TASK_EVENT_SUMMARY_CHARS).refine(isSafeSingleLine, "Task event summary contains unsafe characters.").optional(),
}).strict();
...
export const TaskRecordSchema = z.object({
    ...
    if (record.upstreamRunMissingAt !== undefined) {
        if (record.status !== "unknown"
            ...
            || !record.events.some((event) => event.type === "unknown" && event.timestamp === record.upstreamRunMissingAt)) {
            context.addIssue({
                code: z.ZodIssueCode.custom,
    ...
    if (record.operatorContainedAt !== undefined) {
        if ((record.status !== "unknown" && record.status !== "dispatch_unknown")
            ...
            || !record.events.some((event) => event.type === "operator_contained" && event.timestamp === record.operatorContainedAt)) {
            context.addIssue({
                code: z.ZodIssueCode.custom,
...
export function hashTaskOwnerId(ownerIdentity) {
...
export function hermesSessionIdForTask(taskId) {
...
export function sanitizeTaskTitle(value) {
...
export function sanitizeTaskUsage(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return undefined;
    const sanitized = {};
    for (const [key, rawValue] of Object.entries(value)) {
        if (Object.keys(sanitized).length >= MAX_TASK_USAGE_FIELDS)
            break;
        if (!TaskUsageKeySchema.safeParse(key).success)
            continue;
        if (!TaskUsageValueSchema.safeParse(rawValue).success)
            continue;
...
export function normalizeResourceKeys(values) {
    ...
    if (normalized.length === 0)
        throw new Error("Task requires at least one resource key.");
    return normalized;
}
export function parseTaskTimestamp(value, label = "Task timestamp") {
...
function isSafeSingleLine(value) {
    return value === value.trim() && !UNSAFE_SINGLE_LINE_CHARS.test(value);
}
function isSafeMultiline(value) {
