...
import { n as createCodexInferenceContext } from "./inference-context-Dr_mzvk4.js";
...
async function createCodexInferenceProxy(params) {
	...
	const prepare = (bytes, sampling) => {
		...
		if (rewritten.length > MAX_BODY_BYTES) throw new Error(FAILURE);
		return {
			...prepared,
			bytes: rewritten
		};
	...
	const server = createServer((req, res) => {
		...
		(async () => {
			try {
				...
				if (req.method !== "POST" || active.size >= MAX_CONNECTIONS) throw new Error(FAILURE);
				...
				const body = Buffer.from(encoding === "zstd" ? await compress(prepared.bytes) : prepared.bytes);
				...
				else await guarded.response.body.pipeTo(Writable.toWeb(res), { signal });
			} catch {
	...
	server.on("upgrade", (req, socket, head) => {
		(async () => {
			...
			const close = () => {
				...
				local?.terminate();
				proxyAgent?.destroy();
				if (remote) sockets.delete(remote);
				if (local) sockets.delete(local);
				socket.destroy();
			};
			try {
				const { target, sampling } = resolveTarget(req);
				if (!sampling || active.size >= MAX_CONNECTIONS) throw new Error(FAILURE);
				active.add(controller);
						...
						wss.handleUpgrade(req, socket, head, (accepted) => {
							...
							accepted.on("message", (data, binary) => {
								try {
									...
									if (!remote || remote.readyState !== WebSocket.OPEN || remote.bufferedAmount + prepared.bytes.length > MAX_BODY_BYTES) throw new Error(FAILURE);
							...
							remote.on("message", (data, binary) => {
								if (accepted.readyState !== WebSocket.OPEN || accepted.bufferedAmount + rawBytes(data).length > MAX_BODY_BYTES) {
									close();
									return;
								}
