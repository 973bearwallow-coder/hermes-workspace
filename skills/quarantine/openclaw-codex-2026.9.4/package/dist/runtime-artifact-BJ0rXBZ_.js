import { a as resolvePackagedCodexNativeCommand } from "./managed-binary-D22rNEcR.js";
...
const MAX_ARTIFACT_ID_BYTES = 32768;
...
async function captureFilesystemDescriptor(params) {
	...
	if (process.platform === "win32") {
		...
		if (program.resolution === "node-entrypoint" && entrypoint && !nativeCommand) nativeCommand = resolvePackagedCodexNativeCommand(await fs$1.realpath(entrypoint));
		if (program.resolution === "node-entrypoint" && !nativeCommand) throw new Error("Codex runtime cannot attest a custom Node launcher without its native target");
	...
	const nativePath = await fs$1.realpath(await resolveCommandPath(nativeCommand ?? nativeCandidate, env, spawnCwd));
